...
struct tts_transformer_model {
    ...
    struct ggml_tensor * codec_head = nullptr;     // [hidden_size, codec_vocab_size]
     ...
     // Code predictor input projection
     struct ggml_tensor * code_pred_proj_in = nullptr;       // [hidden_size, code_pred_hidden_size]
     struct ggml_tensor * code_pred_proj_in_bias = nullptr;  // [code_pred_hidden_size]
...
public:
    ...
    // Initialize KV cache
    bool init_kv_cache(int32_t n_ctx, int32_t batch_size = 1);
    ...
    // For batch prefill where prefill_embd is [hidden_size, n_tokens, batch_size]
    ...
    bool forward_codec(int32_t codec_token, int32_t n_past, int32_t batch_size,
                       std::vector<float> & output);
    ...
    bool generate(const int32_t * instruct_tokens, int32_t n_instruct_tokens,
    ...
    bool generate_batch(const std::vector<std::vector<int32_t>> & instruct_tokens,
...
private:
    ...
    bool build_prefill_graph(const int32_t * instruct_tokens, int32_t n_instruct_tokens,
                             const int32_t * text_tokens, int32_t n_tokens, 
                             const float * speaker_embd, 
                             const std::vector<int32_t> * audio_codes,
                             int32_t language_id,
    ...
    struct ggml_cgraph * build_code_pred_step_graph(int32_t n_past, int32_t generation_step, int32_t batch_size = 1);
